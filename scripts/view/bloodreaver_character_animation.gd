...
const IDLE_SCENE: PackedScene = preload("res://resources/characters/bloodreaver/idle3.glb")
const RUN_BEGIN_SCENE: PackedScene = preload(
	"res://resources/characters/bloodreaver/run_2_begin_character.glb"
)
const RUN_LOOP_SCENE: PackedScene = preload(
	"res://resources/characters/bloodreaver/run_2_loop_character.glb"
)
const RUN_END_SCENE: PackedScene = preload(
	"res://resources/characters/bloodreaver/run_2_end_character.glb"
)
const SHIELD_SCENE: PackedScene = preload(
	"res://resources/characters/bloodreaver/shield_1_character.glb"
)
const FIREBALL_CAST_SCENE: PackedScene = preload(
	"res://resources/characters/bloodreaver/fireball_cast_character.glb"
)
const UNDER_ATTACK_SCENE: PackedScene = preload(
	"res://resources/characters/bloodreaver/under_attack_1_character.glb"
)
...
func play_w_cast() -> void:
	if shield_body == null or shield_animation_player == null:
		DebugLogger.log("[WARNING] [bloodreaver_character] shield cast animation is unavailable")
		return
	_hide_all()
...
func play_attack_cast() -> void:
	if shield_body == null or shield_animation_player == null:
		DebugLogger.log("[WARNING] [bloodreaver_character] attack animation is unavailable")
		return
	_hide_all()
	shield_body.visible = true
	state = State.ATTACK_CAST
	shield_animation_player.play(shield_animation_name, -1.0, FIREBALL_CAST_SPEED)


func play_under_attack() -> void:
	if under_attack_body == null or under_attack_animation_player == null:
		DebugLogger.log("[WARNING] [bloodreaver_character] under-attack animation is unavailable")
		return
	_hide_all()
...
func _play_idle() -> void:
	if idle_body == null or idle_animation_player == null:
		DebugLogger.log("[WARNING] [bloodreaver_character] idle animation is unavailable")
		return
	_hide_all()
...
func _on_animation_finished(animation_name: StringName) -> void:
	if animation_name == run_begin_animation_name and state == State.RUN_BEGIN:
		...
		if movement.length_squared() > MOVEMENT_EPSILON_SQUARED:
			...
			_play_idle()
	elif animation_name == shield_animation_name and (state == State.SHIELD_CAST or state == State.ATTACK_CAST):
		_return_to_locomotion()
...
func _add_animation_body(
	...
	if instance == null:
		DebugLogger.log("[WARNING] [bloodreaver_character] failed to instantiate %s" % label)
		return null
	...
	for node in instance.find_children("*", "AnimationPlayer", true, false):
		...
		if animation:
			...
			return player
	DebugLogger.log(
		"[WARNING] [bloodreaver_character] missing animation=%s asset=%s"
