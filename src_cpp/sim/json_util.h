...
inline MapJson parse_map_json(const std::string &text) {
    ...
    auto parse_number = [&](size_t &i) -> float {
        ...
        while (i < text.size() && (std::isdigit(text[i]) || text[i] == '.')) ++i;
        ...
        if (key == "name") {
            ...
            while (pos < text.size()) {
                ...
                while (pos < text.size()) {
                    ...
                    if (wk == "minX") w.minX = wv;
                    else if (wk == "minY") w.minY = wv;
                    else if (wk == "maxX") w.maxX = wv;
                    else if (wk == "maxY") w.maxY = wv;
                    expect(pos, ',');
                }
