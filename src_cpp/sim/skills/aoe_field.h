...
public:
    ...
    void on_cast_complete(entt::registry &reg, entt::entity caster,
        ...
        if (is_bot) skill_dmg *= GameConfig::BotSkillDmgMul;
        ...
        for (auto t : target_view) {
            ...
            if (vec2_length_sq(delta) > range(level) * range(level)) continue;
        ...
        reg.emplace<AoETag>(
            aoe, reg.get<NetworkId>(caster).Value, cs.SkillId,
            range(level), effect_value(level), effect_value(level));
