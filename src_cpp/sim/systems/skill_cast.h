...
namespace detail {
inline void _trigger_effect(entt::registry &reg, CommandBuffer &cb, IdState &ids,
                            double now, entt::entity caster_entity,
                            CastState &cs, const SkillDef &def,
                            Position2D &pos, CombatStats &stats, NetworkId &net);
}
...
inline void skill_cast_system(entt::registry &reg, float dt,
    ...
    auto view = reg.view<PlayerTag, PlayerInputState, SkillComponent, Mana,
                         Position2D, CombatStats, NetworkId>();
    for (auto e : view) {
        ...
        if (cs.RejectTimer > 0.0f) cs.RejectTimer -= dt;
            ...
            case CastState::Phase::Channeling: {
                ...
                if (cs.SubTimer <= 0.0f) {
                    ...
                    for (int i = 0; i < count; ++i) {
                        ...
                        cb.push([arrow_id, spawn_pos, angle, net_id = net.Value,
                                 owner = e, dmg](entt::registry &r) {
                            auto a = r.create();
            ...
            if (best_target != entt::null) {
                ...
                if (hp.Cur <= 0) {
                    ...
                    if (reg.all_of<BotAIState>(best_target))
                        reg.get<BotAIState>(best_target).RespawnTimer = GameConfig::BotRespawnTime;
                    int victim_id = reg.all_of<NetworkId>(best_target)
                        ? reg.get<NetworkId>(best_target).Value : 0;
        ...
        case SkillKind::AoEField: {
            ...
            for (auto t : target_view) {
                ...
                if (hp.Cur <= 0) {
                    ...
                    int victim_id = reg.all_of<NetworkId>(t)
                        ? reg.get<NetworkId>(t).Value : 0;
                    auto kill_view = reg.view<KillEventBuffer>();
                    for (auto k : kill_view)
                        kill_view.get<KillEventBuffer>(k).events.push_back({net.Value, victim_id});
                    if (reg.all_of<Kills>(caster_entity))
