...
inline void attack_fire_system(
    ...
    for (auto e : view) {
        ...
        if (!tag.IsLocal && !reg.all_of<BotTag>(e)) continue;

        if (reg.all_of<StatusEffect>(e)) {
            auto &st = reg.get<StatusEffect>(e);
            if (st.Type == StatusType::Stun && st.Timer > 0.0f) continue;
        }
        ...
        if (at.Target == entt::null) continue;
        if (!reg.valid(at.Target)) continue;
        ...
        if (dist > GameConfig::PlayerAttackRange) continue;
