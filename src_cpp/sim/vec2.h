...
inline Vec2 vec2_clamp_to_map(Vec2 v, float half) {
    return Vec2(
        glm::clamp(v.x, -half, half),
        glm::clamp(v.y, -half, half)
    );
}
...
inline Vec2 resolve_circle_aabb(Vec2 center, float radius, Vec2 bb_min, Vec2 bb_max) {
    Vec2 closest(
    ...
    if (dist_sq < 1e-8f) {
        ...
        if (min_dist == to_left)      return Vec2(bb_min.x - radius, center.y);
        if (min_dist == to_right)     return Vec2(bb_max.x + radius, center.y);
        if (min_dist == to_bottom)    return Vec2(center.x, bb_min.y - radius);
        if (min_dist == to_top)       return Vec2(center.x, bb_max.y + radius);
    }
    return center;
}
